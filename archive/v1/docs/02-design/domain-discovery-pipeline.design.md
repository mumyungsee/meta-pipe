...
> **Plan 문서**: `docs/01-plan/domain-discovery-pipeline.plan.md` (v0.5)
> **PoC 결과**: `test/card-news/pipeline/` (Phase A/B 검증 완료)
...
└── test/{domain-slug}/pipeline/   # 테스트 파이프라인 raw 산출물
...
2. 진입 로직
   ...
   - 캐시 확인: runtime/cache/{domain-slug}/ 존재 여부
...
출력:
  - test/{domain-slug}/pipeline/domain-discovery.md (마크다운 보고서)
  - runtime/cache/{domain}/discovery.json (구조화 데이터)
```
...
출력:
  - test/{domain-slug}/pipeline/pipeline.md (마크다운)
  - test/{domain-slug}/pipeline/pipeline.json (구조화 JSON)
```
...
출력: test/{domain-slug}/pipeline/steps/step-{N}-{name}.md (각 단계 산출물)
```
...
FOR each step in pipeline.steps (순서대로):
  ...
  3. 산출물 저장
     - test/{domain-slug}/pipeline/steps/step-{N}-{name}.md
     - 상태 파일 업데이트
...
Phase A 완료 후:
  - 조사 결과를 파일로 저장
  - SKILL.md에서 "Phase A 결과는 test/{domain-slug}/pipeline/domain-discovery.md 참조" 지시
