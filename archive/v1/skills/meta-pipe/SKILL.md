...
IF runtime/cache/{domain-slug}/ 존재:
  cached_at 확인
  IF 30일 이내:
...
```
1. test/{domain-slug}/pipeline/domain-discovery.md  ← 사람용 마크다��� 보고서
2. runtime/cache/{domain}/                       ← 캐시 (discovery.md + discovery.json + cached_at.json)
...
```
1. test/{domain-slug}/pipeline/pipeline.md    ← 사람이 읽는 마크다운
   - 각 단계: 목적, 할 일, 산출물, 완료 기준, 도메인 주의사항, approval_required
   - 파이프라인 요약표

2. test/{domain-slug}/pipeline/pipeline.json  ← 에이전트가 파싱하는 JSON
...
FOR each step in pipeline.steps (순서대로):
  ...
  2. 산출물 생성
     ...
     - 불확실한 부분에 [검토 필요]
     - 출처 URL 포함
     - 파일 저장: test/{domain-slug}/pipeline/steps/step-{N}-{slug}.md
...
모든 실행의 입출력을 `runtime/logs/{timestamp}.json`에 기록:
...
- Phase A 완료 → 결과를 파일로 저장, "test/{domain-slug}/pipeline/domain-discovery.md 참조" 지시
